...
int answer[4] = {0}, seed = 2973, current_answer_position = 0, answer_completed = 0;
...
{
  TIMER_Open(((TIMER_T *) ((( uint32_t)0x40000000) + 0x10020)), (1UL << 27), 200);
...
int main(void)
{
  ...
  OpenSevenSegment();
	...
	index_5ms=cnt_5ms=cnt_1s=cnt_100ms=0;KEY_Flag = 0;
...
while(1) {	
		...
		if ((KEY_Flag !=0)&& (release ==1)) {
			...
			ASSCII = KEY_Flag+48;
			KEY_Flag =0;
			UART_Write(((UART_T *) ((( uint32_t)0x40000000) + 0x50000)), &ASSCII, 1);
		}		
		...
		if (answer_completed) {
			...
			if (current_line < 4) print_Line(current_line++, result);
